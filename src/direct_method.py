...
def direct_method(
    L: int, L_domain: float = 4.0, source_pos: tuple = (0.6, 1.2)
) -> tuple:
    """
...
def compare_concentrations(
    L=150, source_positions=[(0.6, 1.2), (0.0, 0.0)], titles=None
) -> None:
    """
    ...
    _, axes = plt.subplots(1, len(source_positions), figsize=(12, 5))

    for ax, pos, title in zip(axes, source_positions, titles):
        _, masked_c_grid = direct_method(L, source_pos=pos)
        im = ax.imshow(
            masked_c_grid, extent=(-2, 2, -2, 2), origin="lower", cmap="coolwarm"
        )
        ax.set_title(title)
        ax.set_xlabel("x")
        ax.set_ylabel("y")
